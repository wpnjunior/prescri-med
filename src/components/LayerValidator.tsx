...
/**
 * Mostra contadores de camadas + warnings do Manual Mestre.
 * Usado no PrescriptionPanel.
 */
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white">
        ...
        {(['base', 'modulo', 'ciclo'] as const).map(layer => (
          ...
          >
            ...
            <div
              ...
              style={{ color: counts[layer] > 0 ? LAYER_COLORS[layer] : '#9CA3AF' }}
            >
              {counts[layer]}
            </div>
          </div>
      ...
      {warnings.length > 0 && (
        <div className="border-t border-gray-100 divide-y divide-gray-100">
          {warnings.map((w, i) => {
            const Icon = w.level === 'error' ? AlertCircle : w.level === 'warning' ? AlertCircle : Info;
            const colors =
              w.level === 'error'
            ...
            return (
              <div key={i} className={`${colors.bg} px-3 py-2 flex items-start gap-2`}>
                <Icon size={14} className={`${colors.icon} flex-shrink-0 mt-0.5`} />
                <p className={`text-[11px] ${colors.text} leading-relaxed`}>{w.message}</p>
              </div>
            );
          })}
        </div>
      ...
      {warnings.length === 0 && counts.base > 0 && (
        <div className="bg-green-50 border-t border-green-100 px-3 py-1.5 flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-green-700 font-medium">
            Prescrição segue regras do Manual Mestre ✓
          </span>
        </div>
      )}
    </div>
