        ...
        {slot.entries.map(entry => {
          ...
          return (
            <div
              ...
              className="relative"
              onMouseEnter={() => setHoveredId(entry.instanceId)}
              onMouseLeave={() => setHoveredId(null)}
            >
              <div
                className="flex items-center gap-1 pl-2 pr-1 py-1 rounded-full text-white text-xs font-medium shadow-sm cursor-pointer hover:shadow-md hover:ring-2 hover:ring-white transition-all"
                style={{ backgroundColor: CATEGORY_COLORS[frasco.category] }}
                onClick={(e) => {
                  // Só edita se NÃO clicou no botão X ou no link de compra
                  if ((e.target as HTMLElement).closest('button, a')) return;
                  if (onEditFrasco) onEditFrasco(frasco);
                }}
              ...
              {/* Tooltip de ingredientes ao passar mouse */}
              {isHovered && (
                <div className="absolute z-50 left-0 top-full mt-1 w-72 bg-white border-2 rounded-lg shadow-2xl p-3 pointer-events-none"
                  style={{ borderColor: CATEGORY_COLORS[frasco.category] }}
                >
                  {/* Header */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {frasco.tier && (
                        <p className="text-[10px] text-gray-500 mt-0.5 capitalize">
                          {frasco.tier}
                          {frasco.branded && <span className="ml-1 text-purple-600 font-semibold">✨ Branded</span>}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-0.5 mb-2">
                    <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">
                      Ingredientes ({frasco.ingredients.length})
                    </p>
                  ...
                  {/* Click hint */}
                  {onEditFrasco && (
                    <p className="text-[10px] text-blue-600 mt-2 pt-2 border-t border-gray-100 flex items-center gap-1">
                      <Edit3 size={10} /> Clique para editar
                    </p>
                  )}
                </div>
              )}
