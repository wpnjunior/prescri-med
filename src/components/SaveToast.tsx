 ...
 * Mostra status nas 3 camadas: memória, localStorage, Firebase.
 */
export default function SaveToast({ data, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Etapa 1: memória (imediato)
    const t1 = setTimeout(() => setStage('local'), 200);
    // Etapa 2: localStorage (quase imediato)
    const t2 = setTimeout(() => setStage('cloud'), 600);
    // Etapa 3: Firebase (debounced 2s no context)
    const t3 = setTimeout(() => {
      // Verifica se Firebase está disponível (online)
      if (navigator.onLine) {
        setStage('done');
      } else {
        setStage('cloud-failed');
      }
    }, 3500);
  ...
  const stages = [
    {
      ...
      label: 'Adicionado à sessão',
      done: ['local', 'cloud', 'done', 'cloud-failed'].includes(stage),
    },
    {
      key: 'local',
      icon: stage === 'memory'
        ? <Loader2 size={14} className="animate-spin text-gray-400" />
        : <HardDrive size={14} className={['cloud', 'done', 'cloud-failed'].includes(stage) ? 'text-green-500' : 'text-gray-400'} />,
      label: 'Salvo no navegador (offline)',
      done: ['cloud', 'done', 'cloud-failed'].includes(stage),
    },
    {
      key: 'cloud',
      icon: stage === 'cloud'
        ...
        : <Cloud size={14} className={stage === 'done' ? 'text-green-500' : 'text-gray-400'} />,
      label: stage === 'cloud-failed'
        ? 'Sem internet — sincroniza ao reconectar'
        : 'Sincronizado na nuvem (Firebase)',
      done: stage === 'done',
    },
  ];
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] animate-fade-in pointer-events-auto">
      <div className="bg-white border-2 border-green-400 rounded-xl shadow-2xl p-4 min-w-[320px] max-w-md">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-bold text-sm text-gray-800">
                {TYPE_LABEL[data.type]} {data.action}!
              </p>
        ...
        {/* Status das 3 camadas de salvamento */}
        ...
        {stage === 'done' && (
          <div className="mt-3 pt-2 border-t border-gray-100 bg-green-50 -mx-4 -mb-4 px-4 py-2 rounded-b-xl">
            <p className="text-[10px] text-green-700 font-medium text-center">
              ✓ Não vai perder — salvo em 3 lugares (sessão + navegador + nuvem)
            </p>
          </div>
        )}
        ...
        {stage === 'cloud-failed' && (
          <div className="mt-3 pt-2 border-t border-gray-100 bg-orange-50 -mx-4 -mb-4 px-4 py-2 rounded-b-xl">
            <p className="text-[10px] text-orange-700 font-medium text-center">
              ⚠️ Salvo só no navegador. Quando voltar a internet, sincroniza sozinho.
            </p>
          </div>
        )}
      </div>
